        ...
        with self._db.client().cursor(row_factory=class_row(EventObj)) as cur:
            cur.execute(
                """
                    ...
                    AND event_ts >= CURRENT_DATE - INTERVAL '3 days' --Загружаем только свежие события (за последние 3 дня)
...
class EventsLoader:
    ...
    BATCH_LIMIT = 1542  # Загружаем 100 записей за раз для получения нужного количества.
