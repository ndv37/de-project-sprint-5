...
class TimestampsStg:
    ...
    def list_timestamps(self, rank_threshold: int) -> List[TimestampObj]:
        with self._db.client().cursor(row_factory=class_row(TimestampObj)) as cur:
            cur.execute(
                """
                    SELECT 
                        d.id as id,
                        d.object_value::json->>'delivery_ts' as ts, 
                        extract(year from (d.object_value::json->>'delivery_ts')::timestamp) as year,
                        extract(month from (d.object_value::json->>'delivery_ts')::timestamp) as month,
                        extract(day from (d.object_value::json->>'delivery_ts')::timestamp) as day,
                        ((d.object_value::json->>'delivery_ts')::timestamp)::date as "date",
                        ((d.object_value::json->>'delivery_ts')::timestamp)::time as "time"
...
class TimestampDeliveryLoader:
    WF_KEY = "dds.dm_timestamps_delivery_workflow"
